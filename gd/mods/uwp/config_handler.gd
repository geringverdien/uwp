...
var json_editor_path := (
	"C:/Users/%s/AppData/Roaming/r2modmanPlus-local/WEBFISHING/profiles/Default/GDWeave/configs/"
...
var gdweave_settings: Dictionary
var config_path
...
func _ready():
	...
	var config = JSON.parse(config_file.get_as_text())
	if config.error != OK:
		# TODO: handle error
		print("uh oh spaghetti o's we couldnt read the setting file lets bail")
		return

	gdweave_settings = config.result.duplicate()
	options_buttons.add_child(option_uwp_tab.instance())
	for setting in gdweave_settings.keys():
...
func create_setting_container(key):
	...
	button.selected = int(gdweave_settings.get(key))
...
func on_options_update():
	...
	if not uwp_options:
		return
	var new_settings = gdweave_settings.duplicate()
	...
	for child in uwp_options.get_children():
		...
		new_settings[patch_name] = bool(patch_value)

	var new_file = File.new()
	if new_file.open(config_path, File.WRITE) == OK:
		new_file.store_string(JSON.print(new_settings, "\t"))
		new_file.close()
